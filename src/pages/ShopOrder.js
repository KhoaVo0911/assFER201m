    ...
    dispatch(getShopByUsernameAsyncApi(userObject.username))
      .then((response) => {
        if (response.payload != undefined) {
          dispatch(GetListProductOfSaleAsyncApi(response.payload.id))
            .then((response) => {
              if (response.payload != undefined) {
            ...
            .catch((error) => {});
          dispatch(getEventImgListAsyncApi(response.payload.eventId))
            .then((response) => {
              if (response.payload != undefined) {
              }
            })
      ...
      <div className="mx-auto max-w-2xl px-4 py-16 sm:px-6 sm:py-24 lg:max-w-7xl lg:px-8">
        <div class="flex justify-between items-center">
          ...
          <div class="border-2 rounded-full w-12 h-12 p-[10px] cursor-pointer flex justify-center items-center border-black">
            ...
            <button className="h-6 w-6 bg-blue-400 rounded-full absolute -mt-10 ml-6">
              {arrCart.length}
            </button>
          </div>
        </div>
          ...
          {filteredData &&
            filteredData.map((product, index) => (
              <div key={index} className="  border-2 p-2 ">
                <div
                  className=" w-full overflow-hidden rounded-md bg-gray-200  group-hover:opacity-75 lg:h-80"
                  style={{
                    display: "flex",
                ...
                <div className="mt-4 flex justify-between">
                  ...
                  <p className="text-sm font-medium text-gray-900">
                    {parseToVND(product.price) + " Vnđ"}
                  </p>
                </div>
                <div className="mt-2 flex relative z-10 justify-between">
                  <div
                    onClick={() => handleClickAddToCart(index)}
                  ...
                  <div>
                    <input
                      type="number"
                      value={filteredData[index].quantity}
